...
def build_long_short_one_month(
    ...
    # Filter extreme realized returns to reduce microcap/data-error distortions
    df_month = df_month[
...
required = [ID_COL, DATE_COL, MARKET_COL, 'Price Close', 'Company Market Cap (Millions)']
missing = [c for c in required if c not in df.columns]
if missing:
    raise ValueError(f"Missing required columns: {missing}\nAvailable columns:\n{list(df.columns)}")

# If the forward target is absent, build it from prices while retaining fields needed for backtest realism
if TARGET_COL not in df.columns:
    print(f"'{TARGET_COL}' not found. Building it from Price Close.")
    temp = df[[ID_COL, DATE_COL, MARKET_COL, 'Price Close', 'Company Market Cap (Millions)']].copy()
    temp[DATE_COL] = pd.to_datetime(temp[DATE_COL], errors='coerce')
    temp['Price Close'] = pd.to_numeric(temp['Price Close'], errors='coerce')
    temp['Company Market Cap (Millions)'] = pd.to_numeric(temp['Company Market Cap (Millions)'], errors='coerce')
    ...
    d = temp[[ID_COL, DATE_COL, MARKET_COL, TARGET_COL, 'Price Close', 'Company Market Cap (Millions)']].dropna().copy()
else:
    d = df[[ID_COL, DATE_COL, MARKET_COL, TARGET_COL, 'Price Close', 'Company Market Cap (Millions)']].copy()


# ============================================================
# 4) Cleaning and investability screen
# ============================================================
...
d = d.dropna(subset=[ID_COL, DATE_COL, MARKET_COL, TARGET_COL, 'Price Close', 'Company Market Cap (Millions)']).copy()
d = d.sort_values([ID_COL, DATE_COL])

print('\nWorking sample shape before investability screen:', d.shape)
...
print(
    ...
    f"Company Market Cap >= {MIN_MC_MILLIONS}M): {d.shape}"
)

if d.empty:
    raise ValueError('No rows left after investability screen. Relax MIN_PRICE or MIN_MC_MILLIONS.')
...
# ============================================================
# 13) TASK 5: Backtest long-short portfolio vs HSI and 1M HIBOR
# ============================================================
...
monthly_outlier_stats = backtest.groupby('year_mon')['actual_simple_return'].agg(
    total_names='size',
    extreme_names=lambda s: ((s >= RETURN_CAP_UPPER) | (s <= RETURN_CAP_LOWER)).sum(),
)
print('\nRealized-return outlier summary by month (before filter inside portfolio step):')
print(monthly_outlier_stats.head(12))
print('Average number of extreme realized returns per month:', round(monthly_outlier_stats['extreme_names'].mean(), 2))
...
plt.plot(portfolio_monthly['Date'], portfolio_monthly['cum_portfolio'], label='CAPM Long-Short Portfolio')
...
plt.title('Cumulative Returns: HK CAPM Portfolio vs HSI vs Risk-Free')
plt.xlabel('Date')
...
summary_table = pd.DataFrame({
    'CAPM Long-Short Portfolio': performance_metrics(portfolio_series, rf_series),
...
# 15) Save outputs
# ============================================================
portfolio_monthly.to_csv('capm_fixed_portfolio_monthly_results.csv', index=False)
summary_table.to_csv('capm_fixed_performance_summary.csv')
coef_table.to_csv('capm_fixed_coefficient_table.csv', index=False)

print('\nSaved files:')
print(' - capm_fixed_portfolio_monthly_results.csv')
print(' - capm_fixed_performance_summary.csv')
print(' - capm_fixed_coefficient_table.csv')
