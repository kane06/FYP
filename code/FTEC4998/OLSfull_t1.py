...
# We rely on the columns visible in merged_sample.csv:
# 'ret_adj', 'Mkt-RF', 'SMB', 'HML', 'RMW', 'CMA', 'RF', 'Mom',
# 'market_equity', 'turnover', 'prc', 'vol',
# macro vars: 'tbl', 'tms', 'dfy', 'ntis', 'svar', 'rsvix',
# plus a 'date' column.
...
# Convert 'date' to datetime; let pandas infer format, but coerce bad values to NaT
...
# Excess stock return at time t (ret_adj is already in decimal units)
...
# Feature set for OLS full (similar to RF/GBRT full models)
feature_cols = [
    "Mkt-RF", "SMB", "HML", "RMW", "CMA", "Mom",
...
# Working sample: features at t, target at t+1, plus date
...
X_val = val[feature_cols]        # not used by OLS, but available for comparison
...
# ------------------------------------------------------------
# OLS(full): Linear regression with all predictors in feature_cols
# ------------------------------------------------------------
...
print("Betas (OLS full, merged data):")
print(coefs)
print("Fitted OLS(full) model on merged data.")
...
# "Raw" CAPM beta for reference (OLS on Mkt-RF only, in test set)
...
plt.title("Cumulative Squared Prediction Error – Test Set (OLS full, merged)")
plt.xlabel("Date")
plt.ylabel("Σ(ŷ – y)²")
plt.tight_layout()
plt.show()
...
plt.plot(xs, y_line, linewidth=2, label="OLS full fit (others at mean)")

plt.title("OLS full (merged): Excess Return vs Mkt-RF (Test)")
...
# Mask to central region (typical returns)
...
plt.title("Actual vs Predicted (Test) – OLS full (zoomed)")
plt.grid(alpha=0.3)
plt.tight_layout()
plt.show()

print("Share of points outside |ŷ| or |y| < 0.5:",
      (~mask_plot).mean())
...
print("\nOLS Full 'feature importance' (|coefficient|):")
...
plt.title("OLS Full Feature Importances (absolute coefficients)")
plt.tight_layout()
plt.show()
